...
@lru_cache() # Caching provides ~15x speedup for repeated format checks
...
@lru_cache() # Caching avoids redundant regex processing
...
@lru_cache() # Caching avoids repeated format analysis
def is_datetime(fmt):
    """
...
def is_builtin(fmt):
    """Check if a format code is a builtin. Optimized to O(1) using reverse lookup."""
